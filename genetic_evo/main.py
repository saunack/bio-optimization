...
parser.add_argument('--thresh', default=None, type=float,
                    help='threshold for function function')
...
def run_ga_sort(initial_pop, mutation_prob, elitism, threshold, iterations):
	if iterations is not None:
		run_logs = ga(sort_f, sort_mutation, sort_crossover, sort_initial_pop, elitism=elitism,
                pop=initial_pop, mutation_prob=mutation_prob, iterations=iterations)
	else:
		run_logs = ga(sort_f, sort_mutation, sort_crossover, sort_initial_pop, elitism=elitism,
                pop=initial_pop, mutation_prob=mutation_prob, threshold=threshold)
	return run_logs

if __name__ == "__main__":
	x_opt, run_logs, (x, y) = run_ga_sort(args.pop,args.mutation_prob,args.elitism,
